...
5. **Tracks a user's equipment over time**: the **User History** page shows
   ...
   Each added/removed device also shows who it came from / went to. The
   same page can instead search by **Serial / Service Tag** (Device
   Handover History): one device's chain of holders over time, one row per
   continuous stretch with the same user. Both modes export to Excel.
...
app/
  ...
  static/                          style.css, app.js (auto-uppercase),
                                     trend_chart.js (Dashboard/Branch Detail/
                                     CCTV Dashboard interactive charts), logo
templates_docx/
...
hand. Every Added item with a serial is also annotated with who held it
before ("from"), and every Removed item with who got it next ("to") -
reading one user's history answers "where did this come from / go to"
without a second search. Items with no serial are left unannotated.

The same page's **search by Serial / Service Tag** mode (`?search_by=serial`,
the Device Handover History) uses the same engine directly:
`queries.get_serial_history` returns one snapshot of the device per period
(deduped to the latest batch per period, like analytics.py/diffing.py -
unlike the user mode, which deliberately spans every batch), oldest first,
and `user_history._build_custody_segments` collapses consecutive periods
held by the same `user_id_norm` into one segment (From Period → To Period,
User, Branch, Device, Model, Status). Each segment boundary is a hand-over.
The user mode's from/to annotations are just `_find_neighbor` looking up
the segment before/after the user's own segment in that same list. This
only works for devices with a serial: `importer._asset_key()`'s no-serial
fallback builds `user_id_norm` into the key, so a device without a serial
gets a new identity when it changes hands and there's no chain to follow.
`/user-history/export` honours `search_by` too - the user mode exports
one row per (period, asset) with an Added/Removed column, and the serial
